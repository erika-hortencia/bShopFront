...
import { ScheduleAppointementMonthModel, SelectClientModel, ClientScheduleAppointmentModel, SaveScheduleModel } from '../schedule.models';
  ...
  onScheduleClient(schedule: SaveScheduleModel) {
    if (schedule.startAt && schedule.endAt && schedule.clientId) {
      ...
      this.subscriptions.push(this.httpService.save(request).subscribe(() => {
        this.snackbarManage.show('Appointment included!')
